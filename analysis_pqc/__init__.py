...
def line_regr_with_cuts(x, y, cut_param, debug=False):
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            a, b, r_value, p_value, std_err = linregress(x_fit, y_fit)
...
def analyse_cv(v, c, area=1.56e-6, carrier='electrons', cut_param=0.008, savgol_windowsize=None, min_correl=0.1, debug=False):
    ...
    # spl = CubicSpline(x_norm, y_norm)
    # spl_dev = spl(x_norm, 1)
    spl_dev = savgol_filter(y_norm, window_length=savgol_windowsize, polyorder=1, deriv=1)
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            a_rise, b_rise, r_value_rise, p_value_rise, std_err_rise = linregress(v_rise, c_rise)
            a_const, b_const, r_value_const, p_value_const, std_err_const = linregress(v_const, c_const)
...
def analyse_mos(v, c, cut_param=0.02, debug=False, min_r_value=0.4):
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            # line fits to each region
            a_acc, b_acc, r_value_acc, p_value, std_err = linregress(v_acc, c_acc)
            a_dep, b_dep, r_value_dep, p_value, std_err = linregress(v_dep, c_dep)
            a_inv, b_inv, r_value_inv, p_value, std_err = linregress(v_inv, c_inv)
